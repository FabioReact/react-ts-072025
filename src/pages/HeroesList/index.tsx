...
const HeroesList = () => {
    ...
    const { data: heroes, isLoading, isError, refetch } = useQuery({
    ...
    const getActiveClassnames = (letter: string) => {
        return selectedLetter === letter ? 'text-red-600 cursor-pointer' : 'cursor-pointer';
    }
  ...
  return (
    <section>
        ...
        <div className="flex justify-center gap-2 font-bold">
            {alphabet.map((letter) => (
                <button key={letter} className={getActiveClassnames(letter)} onClick={() => handleLetterChange(letter)}>{letter}</button>
            ))}
        </div>
        <div className="flex flex-wrap justify-center gap-4">
            {isError && <p className="text-red-500 text-center">Error fetching heroes for letter {selectedLetter}</p>}
            <Loading isLoading={isLoading}>
                {heroes?.length === 0 && <p className="text-center">No heroes found for letter {selectedLetter}</p>}
